...
# Black = (0, 0, 0)
# imgBlank = create_blank(506, 1082, rgb_color=Black)

files = glob.glob ("/Users/ranaelgahawy/Desktop/ROV/images/*.png")
img = np.empty(len(files))
# imgStack = stackImages(0.8,([imgBlank,imgBlank],
#                          [imgBlank,imgBlank,imgBlank]))

# cv2.imshow('test', imgBlank)
# cv2.waitKey(0)
...
for i in files:
    ...
    print ("The number of objects in this image: ", num )
    if num == 4:
        top = image_copy
    # else :
...
# Image_Stack = stackImages (0.8,([top]))
cv2.imshow('test', top)
cv2.waitKey(0)